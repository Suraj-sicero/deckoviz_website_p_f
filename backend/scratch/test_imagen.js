...
async function testImagen() {
  ...
  try {
    ...
    if (response.data && response.data.predictions && response.data.predictions.length > 0) {
      ...
      console.log("Imagen 3 success! Received base64 string of length:", base64Data.length);
      const imgBuffer = Buffer.from(base64Data, "base64");
      const outputPath = path.join(__dirname, "../public/uploads/test_imagen_out.jpg");
