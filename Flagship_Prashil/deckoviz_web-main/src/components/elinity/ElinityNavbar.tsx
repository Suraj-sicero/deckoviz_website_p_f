...
const Navbar = () => {
  ...
  const handleNavClick = (item: { name: string; path: string }) => {
    ...
    if (n === 'home') {
      ...
      if (location.pathname === '/elinity') {
        document.getElementById('elinity-steps')?.scrollIntoView({ behavior: 'smooth' });
      } else {
        navigate('/elinity');
        setTimeout(() => document.getElementById('elinity-steps')?.scrollIntoView({ behavior: 'smooth' }), 100);
      }
    } else if (n === 'contact us') {
      navigate('/elinity/contact');
    } else if (n === 'join us') {
      navigate('/elinity/join-us');
    } else {
      if (location.pathname === '/elinity') {
        document.getElementById('elinity-waitlist')?.scrollIntoView({ behavior: 'smooth' });
      } else {
        navigate('/elinity');
        setTimeout(() => document.getElementById('elinity-waitlist')?.scrollIntoView({ behavior: 'smooth' }), 100);
      }
    }
