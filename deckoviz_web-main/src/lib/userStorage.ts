...
export async function compressImageFile(file: File, maxDim = 1600, quality = 0.85): Promise<string> {
  ...
  try {
    ...
    if (raw) {
      ...
      if (Array.isArray(parsed)) {
        // Filter out expired blob: URLs from previous browser sessions
        const validMedia = parsed.filter((item: any) => {
          const url = item.url || item.mediaUrl || item.imageUrl || "";
          return url && !url.startsWith("blob:");
        });
        if (validMedia.length !== parsed.length) {
          localStorage.setItem(userKey, JSON.stringify(validMedia));
        }
        return validMedia;
      }
    }
...
export function saveUserMedia(media: any[]) {
  try {
    ...
    console.warn("Error saving user media, handling quota:", e);
    try {
      const userKey = getUserStorageKey("deckoviz_media");
      const trimmed = media.slice(0, 25);
