      ...
      <React.Fragment key={s}>
        <div className={`flex items-center gap-2 transition-all duration-500 ${i <= current ? 'opacity-100' : 'opacity-30'}`}>
          <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-all
            ${i < current ? 'bg-violet-500 text-white' : i === current ? 'bg-white text-black' : 'bg-white/10 text-white/40'}`}>
            {i < current ? '✓' : i + 1}
          </div>
          <span className="text-xs font-medium uppercase tracking-wider text-white/60">{s}</span>
        </div>
        {i < 2 && <div className={`flex-1 h-px transition-all duration-700 ${i < current ? 'bg-violet-500' : 'bg-white/10'}`} />}
      </React.Fragment>
    ))}
  </div>
);
...
const CreateWorld: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#050508] text-white overflow-hidden selection:bg-violet-500/30"
          ...
          {(stage === 'idle' || stage === 'loading_image') && (
            <motion.div key="idle" initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}
              ...
              <div className="relative mb-8">
                ...
                <div className="relative w-16 h-16 rounded-2xl bg-gradient-to-br from-violet-500 to-blue-600 flex items-center justify-center shadow-2xl">
              ...
              <h1 className="text-5xl md:text-7xl font-black text-center mb-4 leading-none tracking-tight">
                Virtual{' '}
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-violet-400 via-pink-400 to-blue-400">World</span>
              </h1>
              <p className="text-center text-white/40 mb-12 text-base max-w-md mx-auto leading-relaxed">
                ...
                <span className="text-violet-400">Marble 3D Gaussian Splat</span> environment.
              </p>

              {/* Prompt box */}
              <div className="relative group w-full">
                <div className="absolute -inset-px rounded-2xl bg-gradient-to-r from-violet-600/50 via-blue-600/30 to-indigo-600/50 opacity-0 group-focus-within:opacity-100 transition-opacity duration-500 blur-sm" />
                <div className="relative bg-white/[0.04] border border-white/10 rounded-2xl p-1 group-focus-within:border-violet-500/30 transition-all">
                  ...
                  <div className="absolute bottom-3 left-4 right-4 flex items-center justify-between">
                    ...
                    <button onClick={handleGenerate} disabled={!prompt.trim() || stage === 'loading_image'}
                      className="flex items-center gap-2 bg-gradient-to-r from-violet-600 to-blue-600 hover:from-violet-500 hover:to-blue-500 text-white px-6 py-2.5 rounded-xl text-sm font-semibold transition-all disabled:opacity-40 disabled:cursor-not-allowed shadow-lg shadow-violet-500/20">
                      {stage === 'loading_image'
                ...
                {[
                  ...
                  <div key={label} className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center flex-shrink-0">
                      <I className="w-4 h-4 text-violet-400" />
                    </div>
          ...
          {stage === 'preview' && (
            <motion.div key="preview" initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }}
              ...
              <p className="text-center text-white/40 text-sm mb-8">
                This preview will be used as the <span className="text-violet-400">image input</span> for Marble's 3D world generation
              </p>

              <div className="relative group rounded-3xl overflow-hidden border border-white/10 shadow-2xl shadow-violet-500/10 aspect-video mb-8">
                ...
                <div className="absolute top-4 right-4 bg-black/50 backdrop-blur-sm border border-white/10 px-3 py-1 rounded-full">
                  <p className="text-white/60 text-xs font-mono flex items-center gap-1">
                    <Sparkles className="w-3 h-3 text-violet-400" /> Concept Art
                  </p>
                </div>
              </div>

              {/* Marble info panel */}
              <div className="bg-violet-900/20 border border-violet-500/20 rounded-2xl p-5 mb-8">
                <div className="flex items-start gap-3">
                  <div className="w-8 h-8 rounded-lg bg-violet-500/20 flex items-center justify-center flex-shrink-0 mt-0.5">
                    <Globe className="w-4 h-4 text-violet-400" />
                  </div>
                  <div>
                    <p className="text-sm font-semibold text-violet-300 mb-1">Marble World Labs API</p>
                    <p className="text-xs text-white/40 leading-relaxed">
                      ...
                      It generates a real photorealistic 3D Gaussian Splat world with a <code className="text-violet-300">.spz</code> file,
              ...
              <div className="flex items-center gap-4 justify-center">
                ...
                <button onClick={handleBringToLife}
                  className="relative px-10 py-3.5 rounded-xl bg-gradient-to-r from-violet-600 to-blue-600 font-bold text-sm flex items-center gap-2 hover:shadow-lg hover:shadow-violet-500/30 hover:scale-105 transition-all">
            ...
            <motion.div key="loading_world" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              <div className="text-center space-y-2">
                ...
                <div className="flex flex-col items-center gap-1">
                  ...
                  <p className="text-violet-400/30 text-[10px]">Real-time 3D reconstruction is a compute-heavy process</p>
                </div>
              </div>
            </motion.div>
          )}
