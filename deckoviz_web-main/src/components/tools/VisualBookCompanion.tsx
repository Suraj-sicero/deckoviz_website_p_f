...
const API_BASE = "http://localhost:5000/api/wizzy";
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-12">
          <div>
            ...
            <p className="text-sm text-purple-600 font-bold tracking-[0.2em] uppercase">Powered by Google Gemini 1.5 Pro • Production v1.0</p>
          </div>
            ...
            <motion.div key="setup" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              <div className="lg:col-span-8 space-y-8">
                <div className="flex gap-2 p-1 bg-gray-100 rounded-2xl w-fit border border-gray-200/50">
                  <button onClick={() => setInputMethod("pdf")} className={`px-6 py-2 rounded-xl text-xs font-bold uppercase tracking-widest transition-all ${inputMethod === "pdf" ? 'bg-white text-purple-600 shadow-lg' : 'text-gray-400 hover:text-gray-600'}`}>Upload PDF</button>
                  <button onClick={() => setInputMethod("text")} className={`px-6 py-2 rounded-xl text-xs font-bold uppercase tracking-widest transition-all ${inputMethod === "text" ? 'bg-white text-purple-600 shadow-lg' : 'text-gray-400 hover:text-gray-600'}`}>Paste Text</button>
                </div>

                <div className="bg-white rounded-3xl p-8 border border-gray-100 shadow-xl shadow-purple-500/5 min-h-[350px] flex flex-col relative overflow-hidden">
                  ...
                  {inputMethod === "text" ? (
                    <textarea className="flex-1 w-full p-8 rounded-2xl border border-gray-100 bg-gray-50/30 focus:ring-4 focus:ring-purple-100 focus:bg-white outline-none transition-all text-sm leading-relaxed text-gray-600 resize-none font-medium" placeholder="Paste your narrative here..." value={rawText} onChange={(e) => setRawText(e.target.value.slice(0, MAX_CHARS))} />
                  ) : (
                    <label className="relative flex-1 flex flex-col items-center justify-center w-full border-2 border-dashed border-purple-100 rounded-3xl hover:bg-purple-50/30 transition-all cursor-pointer group">
                      <div className="flex flex-col items-center justify-center pt-5 pb-6 text-center">
                        <div className="w-16 h-16 rounded-2xl bg-purple-50 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform shadow-sm text-3xl">📁</div>
                ...
                <div className="bg-white rounded-3xl p-8 border border-gray-100 shadow-xl shadow-purple-500/5">
                  <h3 className="text-sm font-black text-gray-400 uppercase tracking-widest mb-6">Base Artistic Direction</h3>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {STYLES.map(s => (
                      <button key={s.id} onClick={() => setSelectedStyle(s)} className={`p-4 rounded-2xl border transition-all text-center ${selectedStyle.id === s.id ? 'border-purple-500 bg-purple-50/50 shadow-lg' : 'border-gray-50 bg-white'}`}>
                        <div className="text-3xl mb-2">{s.icon}</div>
                        <div className={`font-black text-[10px] uppercase tracking-widest ${selectedStyle.id === s.id ? 'text-purple-700' : 'text-gray-500'}`}>{s.name}</div>
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div className="bg-white rounded-3xl p-8 border border-gray-100 shadow-xl shadow-purple-500/5">
                  ...
                  <div className="mb-8">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {ASPECT_RATIOS.map(ratio => (
                        <button key={ratio.id} onClick={() => setAspectRatio(ratio)} className={`py-3 rounded-xl border text-[10px] font-black uppercase tracking-widest transition-all ${aspectRatio.id === ratio.id ? 'bg-purple-600 text-white border-purple-600' : 'bg-gray-50 text-gray-400'}`}>{ratio.label}</button>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="mb-8 border-t border-gray-50 pt-6">
                    <div className="flex justify-between items-center mb-4">
                      ...
                      <button onClick={() => setHasBorders(!hasBorders)} className={`w-10 h-5 rounded-full relative transition-all ${hasBorders ? 'bg-purple-600' : 'bg-gray-200'}`}><div className={`absolute top-1 w-3 h-3 rounded-full bg-white transition-all ${hasBorders ? 'left-6' : 'left-1'}`} /></button>
                    </div>
                    {hasBorders && (
                      <div className="flex gap-2">
                        {BORDER_COLORS.map(color => (
                          <button key={color.id} onClick={() => setBorderColor(color)} className={`w-8 h-8 rounded-lg border-2 transition-all ${borderColor.id === color.id ? 'border-purple-500 scale-110' : 'border-transparent'}`} style={{ backgroundColor: color.color }} title={color.label} />
                        ))}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="mb-10 border-t border-gray-50 pt-6">
                    <div className="flex justify-between mb-2">
                      ...
                      <span className="text-xs font-black text-purple-600 italic">{numPages}</span>
                    </div>
                    <input type="range" min="10" max="200" step="10" value={numPages} onChange={(e) => setNumPages(parseInt(e.target.value))} className="w-full h-1.5 bg-gray-100 rounded-lg appearance-none cursor-pointer accent-purple-500" />
                  ...
                  <button disabled={(inputMethod === "pdf" ? !file : rawText.trim().length < 20) || isGenerating} onClick={startGeneration} className="w-full py-5 rounded-2xl bg-purple-600 text-white font-black uppercase tracking-[0.2em] shadow-xl hover:bg-purple-500 transition-all">Launch Pipeline</button>
          ...
          {viewMode === "generating" && (
            <motion.div key="generating" className="flex flex-col items-center justify-center py-20 text-center">
              <div className="relative w-48 h-48 mb-12">
                <div className="absolute inset-0 rounded-full border-[8px] border-purple-50" />
                <motion.div className="absolute inset-0 rounded-full border-[8px] border-purple-600 border-t-transparent shadow-[0_0_20px_rgba(124,58,237,0.4)]" animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: "linear" }} />
              ...
              <p className="text-xs font-mono text-purple-600 uppercase tracking-widest animate-pulse max-w-lg mx-auto">"{currentStatus}"</p>
            </motion.div>
          )}
            ...
            <motion.div key="result" className="space-y-12">
              <div className="flex justify-between items-end">
                <div>
                  ...
                  <p className="text-[10px] text-purple-600 font-black uppercase tracking-widest">{beats.length} High-Fidelity Pages Synthesized</p>
                </div>
                  ...
                  <div key={i} className={`relative rounded-[2rem] overflow-hidden shadow-2xl bg-white transition-all duration-700 ${aspectRatio.class} ${hasBorders ? 'p-4' : ''}`} style={{ backgroundColor: hasBorders ? borderColor.color : 'white' }}>
                    <div className="w-full h-full rounded-[1.5rem] overflow-hidden relative group">
                      ...
                      <div className="absolute inset-0 bg-black/70 opacity-0 group-hover:opacity-100 transition-opacity p-8 flex flex-col justify-end">
                        <p className="text-[10px] font-black text-purple-400 uppercase mb-2">Gemini Analysis</p>
