...
import { MORE_PROMPT_GROUPS, MorePromptTemplate, MorePromptSection } from "./lib/more-prompt-templates"
...
const CATEGORY_ICONS: Record<string, React.ComponentType<any>> = {
  ...
  "Posters": Layout,
  "Stories": BookOpen,
  "Narrative experiences": Sparkles,
  "Music & sound": Music,
  "Video & motion": Video,
  "Memory & personal": Heart,
  "Ambient & living": Sun,
  "Seasonal & celebrations": CalendarDays,
  "Kids & family": Users,
  "Reflective & philosophical": Compass,
  "Miscellaneous & wildcard": HelpCircle
}

export function MoreTemplatesModal({ isOpen, onClose, onSelect }: MoreTemplatesModalProps) {
  const [activeCategory, setActiveCategory] = useState<string>("Artwork")
  ...
  const searchResults = useMemo(() => {
    ...
    const matches: { sectionTitle: string; template: MorePromptTemplate }[] = []

    MORE_PROMPT_GROUPS.forEach((group) => {
  ...
  const currentGroup = MORE_PROMPT_GROUPS.find((g) => g.category === activeCategory)
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 border-b border-white/[0.08]">
          <div className="flex-1 min-w-0">
            <h2 className="text-xl font-bold font-serif flex items-center gap-2 text-white">
              ...
              <span>120 More Home &amp; Personal Templates</span>
            </h2>
            <p className="text-xs text-slate-400 mt-1">
              Select or copy seed prompts structured for home environment personalization.
            </p>
          </div>
